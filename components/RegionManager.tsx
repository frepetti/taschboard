...
import { Plus, Edit2, Trash2, Map, Save, X } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl text-content-main font-bold flex items-center gap-2">
        ...
        <button
          onClick={() => openModal()}
          className="flex items-center gap-2 bg-theme-primary hover:brightness-95 text-white px-4 py-2 rounded-lg transition-colors shadow-sm"
        >
      ...
      {loading ? (
        <div className="flex justify-center py-12">
          <LoadingSpinner size="lg" text="Cargando regiones..." />
        </div>
      ) : regions.length === 0 ? (
        <div className="text-center py-12 bg-surface-card rounded-xl border border-border-subtle">
          <Map className="w-12 h-12 text-content-muted mx-auto mb-4" />
          <p className="text-content-muted">No hay regiones configuradas</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {regions.map((region) => (
            <div
              key={region.id}
              className="bg-surface-card border border-border-subtle rounded-xl p-6 hover:border-theme-primary/40 transition-all group shadow-sm"
            >
              ...
              <h3 className="text-lg text-content-main font-semibold mb-2">{region.nombre}</h3>
              <p className="text-content-muted text-sm h-10 overflow-hidden text-ellipsis">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-surface-card border border-border-subtle rounded-2xl w-full max-w-md shadow-2xl overflow-hidden text-content-main">
            <div className="flex items-center justify-between p-6 border-b border-border-subtle">
              <h3 className="text-xl text-content-main font-bold">
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-surface-card-subtle border border-border-subtle rounded-lg px-4 py-2 text-content-main placeholder:text-content-muted focus:outline-none focus:border-theme-primary focus:ring-1 focus:ring-theme-primary transition-colors"
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-surface-card-subtle border border-border-subtle rounded-lg px-4 py-2 text-content-main placeholder:text-content-muted focus:outline-none focus:border-theme-primary focus:ring-1 focus:ring-theme-primary transition-colors h-24 resize-none"
